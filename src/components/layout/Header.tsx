...
export function Header() {
  ...
  const pendingAlerts = (crisisOrders ?? []).filter((c: any) => c.status === 'pendiente').length
    ...
    <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="relative p-2 hover:bg-slate-100 rounded-lg">
          ...
          {pendingAlerts > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 rounded-full text-white text-[10px] flex items-center justify-center">{pendingAlerts}</span>}
        </button>
        <DropdownMenu>
